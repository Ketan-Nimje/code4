...
function myJQueryCode(jQuery) {
    if (!jQuery("link[href='" + BASE_URLINS + "assets/gallerysetting.css']").length > 0) {
        ...
        linkx.type = 'text/css';
        linkx.href = BASE_URLINS + 'assets/gallerysetting.css?shop=' + shop_name;
    ...
    if (jQuery('#wc-instagram-feed').length) {
        ...
        function getdata() {
            ...
            if (shop_name != '' && shop_name != undefined && shop_name != null && gallery_type != '' && gallery_type != undefined && gallery_type != null) {
        ...
        function InstaProductTagFeed(shop_data, datalist) {
            ...
            if (mediadata.length > 0) {
                ...
                if (shop_data.display_type == '1') {
                    mediaPost += '<div class="th-insta-container" ><ul class="' + shop_data.ul_class + '"  data-show="' + shop_data.cell_size + '" data-fixed="false" style="margin-left:-10px;margin-right:-10px;" >';
                } else {
                    mediaPost += '<div class="th-insta-container" style="padding-left:' + gridmargin + 'px;padding-right:' + gridmargin + 'px;"><ul class="' + shop_data.ul_class + ((shop_data.layout_type == "0") ? no_of_column : '') + '" style="margin-left:-' + gridmargin + 'px;margin-right:-' + gridmargin + 'px;margin-bottom:-' + gridmargin + 'px;">';
                }
        ...
        function InstaProductFeed(shop_data, datalist, is_loadmore = false) {
            ...
            if (!is_loadmore) {
                ...
                if (shop_data.display_type == '1') {
                    ...
                    mediaPost += '<ul class="' + shop_data.ul_class + ((shop_data.layout_type == "0") ? no_of_column : '') + ' th-insta-view-ul"  style="margin-left:-' + gridmargin + 'px;margin-right:-' + gridmargin + 'px;margin-bottom:-' + gridmargin + 'px;" >';
                }
                mediadata = datalist.data;
            } else {
            ...
            if (is_loadmore) {
                ...
                jQuery('.th-insta-view-ul li:last').after(mediaPost).show().fadeIn("slow");
                jQuery('.load_more_feed').remove();
                jQuery('#wc-instagram-feed').after('<div style="width: 100%;padding: 15px;text-align: center;" class="load_more_feed"><button class="btn btn-dark" id="load_more_feed" data-shopname="' + shop_name + '">Load more</button></div>');
               
            } else {
                mediaPost += '</ul>' + '</div>';
                mediaPost += '</div>';
                if (shop_data.display_type == '0') {
                    if (shop_data.show_load_more == "1" && datalist.data.length > 0) {
...
//            console.log("mediadata.length", mediadata.length, "datalist.tatal_record", datalist.tatal_record);
        ...
        jQuery('body').on('click', '#load_more_feed', function (e) {
            e.preventDefault();
            pageno = (pageno + 1);
            jQuery('#load_more_feed').text('loading...');
            getdata();
            ...
            if (mediadata[i]) {
                ...
                if (productdata.length > 0) {
                    jQuery.each(productdata, function (x, y) {
                        ...
                        modelproduct += '<figure><a  href="javascript:void(0);" onclick="calculation(' + mediadata[i].insta_feed_id + ',\'1\',\'https://' + shop_data.domain + '/products/' + y.shopify_handle_name + '\');"  href="" class="image__body"><img src="' + y.product_image + '" alt=""></a>  </figure>';
                        ...
                        modelproduct += '</div>';
                        modelproduct += '<a class="th-ig-buy-now th-ig-font"   href="javascript:void(0);"   onclick="calculation(' + mediadata[i].insta_feed_id + ',\'1\',\'https://' + shop_data.domain + '/products/' + y.shopify_handle_name + '\');">' + bybtn + '</a>';
                        modelproduct += '</div>';
                        modelproduct += '</li>';
                        if (gallery_type == "InstaProductFeed") {
